...
export const initializeUiConfig = async ({ uiConfigPath, statusStylesDir, globalDir }: UiPaths): Promise<void> => {
  ...
  await ensureDir(statusStylesDir);
  if (!(await pathExists(uiConfigPath))) {
    await writeJson(uiConfigPath, DEFAULT_UI_CONFIG, { spaces: 2 });
  }
};
...
export const readUiConfig = async (paths: UiPaths): Promise<UiConfig> => {
  if (!(await pathExists(paths.uiConfigPath))) return { ...DEFAULT_UI_CONFIG };
  return { ...DEFAULT_UI_CONFIG, ...(await readJson(paths.uiConfigPath)) };
};
...
export const writeUiConfig = async (paths: UiPaths, partial: Partial<UiConfig>): Promise<UiConfig> => {
  ...
  await writeJson(paths.uiConfigPath, updated, { spaces: 2 });
  return updated;
};
...
export const ensureCustomStatusStyle = async (
  ...
  if (!(await pathExists(targetPath))) {
    await writeJson(targetPath, template, { spaces: 2 });
  }
