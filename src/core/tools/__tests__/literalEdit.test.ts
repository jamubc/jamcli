...
beforeEach(async () => {
  projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'jamcli-literal-'));
});
...
test('edit inserts dollar sequences literally', async () => {
  await fs.writeFile(file('run.sh'), '#!/bin/sh\necho PID\n');
...
test('replace_all replaces every occurrence and reports the count', async () => {
  ...
  expect(await fs.readFile(file('a.txt'), 'utf-8')).toBe('value = 1\ny = value\nz = value\n');
});

test('occurrence replaces only the chosen match', async () => {
  await fs.writeFile(file('a.txt'), 'alpha\nbeta\nalpha\n');
  const result = await registry().execute(
    'edit',
  ...
  expect(result.success).toBe(true);
  expect(await fs.readFile(file('a.txt'), 'utf-8')).toBe('alpha\nbeta\ngamma\n');
});
...
test('a CRLF file keeps CRLF when the snippet is written with LF', async () => {
  await fs.writeFile(file('win.txt'), 'one\r\ntwo\r\nthree\r\n');
  const result = await registry().execute(
    'edit',
  ...
  expect(result.success).toBe(true);
  expect(await fs.readFile(file('win.txt'), 'utf-8')).toBe('uno\r\ndos\r\ndos-b\r\nthree\r\n');
});
